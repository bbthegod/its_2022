...
export async function gen(req, res) {
  const studentCode = "admin"
  const studentName = "admin"
  const studentClass = "HTTT1-K12"
  const studentPhone = "0123456789"
  const password = "spithaui"
  const duplicate = await User.findOne({ studentCode });
  if (duplicate) return res.status(httpStatus.CONFLICT).json('StudentCode already exist');
  let user = new User({ studentCode, studentName, studentClass, studentPhone, password, role: 'admin' });
  user = await user.save();
  return res.json(user);
}